...
# file COPYING or http://www.opensource.org/licenses/mit-lice   nse.php.
...
class ReorgStakeTest(BlackHatTestFramework):
    ...
    def check_money_supply(self, expected_blkc):
        # verify that nodes have the expected BLKC supply
        blkc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(blkc_supply, [DecimalAmt(expected_blkc)] * self.num_nodes)
