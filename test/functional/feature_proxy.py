...
"""Test blkcd with different proxy configuration.

Test plan:
- Start blkcd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on blkcd side:
...
- Create blkcds that connect to them
- Manipulate the blkcds using addnode (onetry) an observe effects
...
class ProxyTest(BlackHatTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        if self.have_ipv6:
            ...
            assert isinstance(cmd, Socks5Command)
            # Note: blkcd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
