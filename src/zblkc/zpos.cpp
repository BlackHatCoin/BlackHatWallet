...
bool CLegacyZBlkcStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZBlkcStake from non zc-spend", __func__);
    ...
    *this = CLegacyZBlkcStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zblkc stake origin", __func__);

    // All good
    return true;
}

CLegacyZBlkcStake::CLegacyZBlkcStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZBlkcStake::GetIndexFrom() const
{
...
CAmount CLegacyZBlkcStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZBlkcStake::GetUniqueness() const
{
...
bool CLegacyZBlkcStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zBLKC stake block: height %d outside range", __func__, nHeight);
