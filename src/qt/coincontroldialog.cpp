...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select BLKC Outputs to Spend" :
            "Select Shielded BLKC to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::BLKC;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show BLKC address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
