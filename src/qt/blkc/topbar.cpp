...
#include "qt/blkc/topbar.h"
#include "qt/blkc/forms/ui_topbar.h"
...
#include "qt/blkc/balancebubble.h"
...
TopBar::TopBar(BLKCGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopBlkc, ui->labelAmountTopShieldedBlkc}, "amount-small-topbar");
    setCssProperty({ui->labelAmountBlkc}, "amount-topbar");
    setCssProperty({ui->labelPendingBlkc, ui->labelImmatureBlkc}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("BLKC")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // BLKC Total
    QString totalBlkc = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // BLKC
    // Top
    ui->labelAmountTopBlkc->setText(totalTransparent);
    ui->labelAmountTopShieldedBlkc->setText(totalShielded);
    // Expanded
    ui->labelAmountBlkc->setText(totalBlkc);
    ui->labelPendingBlkc->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureBlkc->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
