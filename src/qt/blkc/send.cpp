...
SendWidget::SendWidget(BLKCGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-blkc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onBLKCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onBLKCSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent BLKCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        inform(tr("You don't have any transparent BLKCs to shield."));
    }
}
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(blkcAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
