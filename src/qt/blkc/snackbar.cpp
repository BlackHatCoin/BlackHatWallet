...
#include "qt/blkc/forms/ui_snackbar.h"
#include "qt/blkc/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BLKCGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BLKCGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
