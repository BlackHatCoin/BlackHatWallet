...
{
    return QCoreApplication::translate("blkc-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in blkc.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in blkc.qrc)
...
/** Class encapsulating BlackHat Core startup and shutdown.
...
/** Main BlackHat application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (BLKCGUI)
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &BLKCGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // BlackHat: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BLKCGUI::handlePaymentRequest);
        connect(window, &BLKCGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(blkc_locale);
    Q_INIT_RESOURCE(blkc);
    ...
    // blkc: links repeatedly have their payment requests routed to this process:
