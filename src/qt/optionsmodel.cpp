...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BLKC);
    ...
    if (settings.contains("nAnonymizeBlkcAmount") || reset)
        gArgs.SoftSetArg("-anonymizeblkcamount", settings.value("nAnonymizeBlkcAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in blkc.cpp during shotdown to also remove the window positions
