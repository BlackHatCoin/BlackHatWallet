...
#if defined(NDEBUG)
#error "BlackHat cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ublkc) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k BLKC
    ...
    if (nHeight > 1296000) return 2.14  * COIN;
    if (nHeight > 1166400) return 2.68  * COIN;
    if (nHeight > 1036800) return 3.35  * COIN;
    if (nHeight > 907200)  return 4.19  * COIN;
    if (nHeight > 777600)  return 5.24  * COIN;
    if (nHeight > 648000)  return 6.55  * COIN;
    if (nHeight > 518400)  return 8.19  * COIN;
    if (nHeight > 388800)  return 10.24 * COIN;
    if (nHeight > 259200)  return 12.8  * COIN;
    if (nHeight > 129600)  return 16    * COIN;
    if (nHeight > 2000)    return 20    * COIN;
    if (nHeight > 100)     return 0.1   * COIN; // Fair start till block 2000
    if (nHeight > 1)       return 450   * COIN; // Premine for referral program and presale
...
{
    util::ThreadRename("blkc-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC_V2) &&
        ...
        if (pindex->nHeight % 50000 == 0) {
            ZBLKCModule::CleanCoinSpendsCache();
        }
                                     ...
                                     std::set<CBigNum>& spent_serials)
{
    ...
    const bool zblkcActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC);
    const bool publicZblkcActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_PUBLIC);
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPublicSpend && (!publicZblkcActive || v5Active)) {
            ...
            if (isPrivZerocoinSpend && (!zblkcActive || publicZblkcActive)) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZBLKCModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
                    spend = ZBLKCModule::TxInToZerocoinSpend(in);
                }
        ...
        for (auto it = bl.vtx.rbegin(); it != bl.vtx.rend(); ++it) {
            ...
            for (const CTxIn& in: tx->vin) {
                ...
                if (!in.IsZerocoinSpend()) {
                    ...
                    // zerocoin serial
                    const CBigNum& s = ZBLKCModule::TxInToZerocoinSpend(in).getCoinSerialNumber();
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, const FlatFilePos* dbp) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (isPoS) {
        ...
        if (coinstake_in.IsZerocoinSpend()) {
            libzerocoin::CoinSpend spend = ZBLKCModule::TxInToZerocoinSpend(coinstake_in);
            if (!ContextualCheckZerocoinSpend(coinstake, &spend, pindex->nHeight)) {
                return state.DoS(100,error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                        coinstake.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zblkc");
            }
        }
