...
*Setup instructions for a gitian build of BlackHat Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BlackHat
...
to the BlackHat GitHub release page.
...
- [Building BlackHat Core](#building-blkc-core)
...
gpg --output ${VERSION}-osx-unsigned/$NAME/blackhat-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/blackhat-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/blackhat-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/blackhat-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/BlackHatCoin/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/blackhat-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/blackhat-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/blackhat-osx-*-build.assert
```
